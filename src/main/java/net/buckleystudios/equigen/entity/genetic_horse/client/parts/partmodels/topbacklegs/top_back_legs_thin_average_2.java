...
public class top_back_legs_thin_average_2 extends MultipartBackTopLegModel<GeneticHorseEntity> {
	...
	public static LayerDefinition createBodyLayer(GeneticValues.LEG leg) {
		...
		PartDefinition cube_r1 = top_back_legs_thin_average_2_top_back_front.addOrReplaceChild("cube_r1", CubeListBuilder.create().texOffs(uvXOffset, uvYOffset).mirror().addBox(-1.1444F, -3.0F, -1.0F, 2.0F, 2.0F, 10.0F, new CubeDeformation(0.0F)).mirror(false), PartPose.offsetAndRotation(0.1444F, 3.0747F, 3.2475F, 1.9199F, 0.0F, 0.0F));
		...
		PartDefinition cube_r2 = top_back_legs_thin_average_2_top_back_back.addOrReplaceChild("cube_r2", CubeListBuilder.create().texOffs(uvXOffset, uvYOffset).mirror().addBox(-0.8556F, -1.0F, -0.92F, 2.0F, 2.0F, 9.0F, new CubeDeformation(0.0F)).mirror(false), PartPose.offsetAndRotation(-0.1444F, 3.4951F, 0.7748F, 1.789F, 0.0F, 0.0F));
	...
	public Part getCubeDimensions() {
		return new Part("top_back_legs_thin_average_2", new ArrayList<>(
				Arrays.asList(
						new Block(2, 2, 10),
						new Block(2, 2, 9))
		));
