...
public class top_back_legs_thick_long_2 extends MultipartBackTopLegModel<GeneticHorseEntity> {
	...
	public static LayerDefinition createBodyLayer(GeneticValues.LEG leg) {
		...
		PartDefinition cube_r1 = top_back_legs_thick_long_2_top_back_front.addOrReplaceChild("cube_r1", CubeListBuilder.create().texOffs(uvXOffset, uvYOffset).mirror().addBox(-1.1444F, -3.0F, -1.0F, 2.0F, 3.0F, 10.0F, new CubeDeformation(0.0F)).mirror(false), PartPose.offsetAndRotation(0.1444F, 3.3638F, 2.6334F, 1.8762F, 0.0F, 0.0F));
		...
		PartDefinition cube_r2 = top_back_legs_thick_long_2_top_back_back.addOrReplaceChild("cube_r2", CubeListBuilder.create().texOffs(uvXOffset, uvYOffset).mirror().addBox(-0.8556F, -1.0F, -2.92F, 2.0F, 3.0F, 11.0F, new CubeDeformation(0.0F)).mirror(false), PartPose.offsetAndRotation(-0.1444F, 2.6276F, -0.0444F, 1.7453F, 0.0F, 0.0F));
	...
	public Part getCubeDimensions() {
		return new Part("top_back_legs_thick_long_2", new ArrayList<>(
				Arrays.asList(
						new Block(2, 3, 10),
						new Block(2, 3, 11))
		));
